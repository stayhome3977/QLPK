...
def forgot_password(email: str = Body(..., embed=True)):
    return {"message": f"Đã ghi nhận yêu cầu quên mật khẩu cho {email}"}


@router.post("/reset-password", response_model=MessageResponse)
def reset_password(email: str = Body(..., embed=True), new_password: str = Body(..., embed=True), db: Session = Depends(get_db)):
